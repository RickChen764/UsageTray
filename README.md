...
- 更新窗口汇总展示当前版本之后的全部 Release Notes，API 限流时自动改用公开 Feed
- 专用更新窗口展示 Release Notes：清理常见 Markdown，支持独立滚动阅读
- 更新窗口直接展示 Release Notes 详情，不提供额外跳转入口
- 更新窗口可在系统浏览器中打开 GitHub 完整 Releases 更新日志
- 右键菜单点击外部或切换窗口后自动关闭；更新过程中显示阶段/进度并禁用重复操作
- Hover 使用分组信息卡突出余额与今日费用，请求、Token、性能和模型明细按层级展示
- Hover 使用紧凑布局提高信息密度，并强制保持在当前屏幕工作区内
